...
export function EditPatientDialog({ patient, open, onOpenChange }: EditPatientDialogProps) {
  ...
  useEffect(() => {
    if (patient) {
      form.reset({
        ...
        insurance_provider: patient.insurance_provider_name || '',
        stage: patient.stage,
      });
    }
  }, [patient, form]);
  ...
  const onSubmit = async (data: PatientFormData) => {
    ...
    try {
      ...
      const description = error?.message || error?.error || JSON.stringify(error) || 'Failed to update patient. Please try again.';
      toast({
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              <Button type="submit" disabled={updatePatient.isPending} className="gradient-primary border-0">
