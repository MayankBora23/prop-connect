...
export function ChatHeaderControls({
  ...
  currentProfileId,
  children,
}: ChatHeaderControlsProps & { children?: React.ReactNode }) {
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto justify-end">
        {/* Row 1 (mobile only): AI Handling status badge — solo */}
        ...
        {/* Row 2 on mobile / inline on desktop: status badge (desktop) + AI toggle + Assign */}
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          {/* Badge visible only on sm+ (desktop inline) */}
          <Badge variant="outline" className={cn('gap-1 text-xs hidden sm:flex', statusCfg.className)}>
            <StatusIcon className="w-3 h-3" />
            {statusCfg.label}
          </Badge>
        ...
        {/* Row 3 on mobile / inline on desktop: History and children dropdown controls */}
