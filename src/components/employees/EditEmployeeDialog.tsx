...
// Helper function to format salary for display in form
function formatSalaryForForm(salary: number | null): string {
  if (!salary) return '';
...
export function EditEmployeeDialog({ open, onOpenChange, employee }: EditEmployeeDialogProps) {
  ...
  useEffect(() => {
    if (employee) {
      form.reset({
        ...
        salary: formatSalaryForForm(employee.salary),
  ...
  const onSubmit = async (data: EmployeeFormData) => {
    ...
    try {
      ...
      const description = error?.message || error?.error || JSON.stringify(error) || 'Failed to update employee. Please try again.';
      toast({
