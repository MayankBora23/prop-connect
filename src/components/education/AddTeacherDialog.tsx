...
export function AddTeacherDialog({ open, onOpenChange }: AddTeacherDialogProps) {
  ...
  const onSubmit = async (data: TeacherFormData) => {
    try {
      ...
      const description =
        error?.message || error?.error || JSON.stringify(error) || 'Failed to add teacher. Please try again.';
      toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new teacher/faculty member with their qualifications, subjects, and details.
          </DialogDescription>
        </DialogHeader>
