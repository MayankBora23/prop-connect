...
export function AddCourseDialog({ open, onOpenChange }: AddCourseDialogProps) {
  ...
  const onSubmit = async (data: CourseFormData) => {
    try {
      ...
      const description =
        error?.message || error?.error || JSON.stringify(error) || 'Failed to add course. Please try again.';
      toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new course with detailed information including type, duration, fees, and faculty assignment.
          </DialogDescription>
        </DialogHeader>
