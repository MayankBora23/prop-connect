...
export function Sidebar({ activeTab, onTabChange, onCollapsedChange }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 z-40 h-screen h-[100dvh] bg-sidebar flex flex-col overflow-hidden',
          'transition-all duration-300',
          // Desktop behavior
          !isMobile && (collapsed ? 'w-16' : 'w-64'),
          // Mobile behavior — always w-64, slides in/out
          isMobile && 'w-64',
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4 border-b border-sidebar-border">
          ...
          {isMobile && (
            <button
              onClick={() => setMobileOpen(false)}
              className="p-1.5 rounded-lg hover:bg-sidebar-accent text-sidebar-foreground transition-colors md:hidden"
            >
              <X className="w-5 h-5" />
            </button>
          )}
